...
export function exportToCsv(medicines: Medicine[], purchases: PurchaseItem[] = []): void {
  ...
  const rows = medicines.map((m) => {
    ...
    return [
      ...
      purchase && typeof purchase.actualQuantity === 'number' ? String(purchase.actualQuantity) : '',
      purchase && purchase.actualPurchaseDate ? purchase.actualPurchaseDate : '',
      purchase && purchase.batchExpireDate ? purchase.batchExpireDate : '',
      purchase && purchase.completionNotes ? purchase.completionNotes : '',
