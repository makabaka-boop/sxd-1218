...
export function filterMedicines(list: Medicine[], filters: FilterState, purchases: PurchaseItem[] = [], medicines: Medicine[] = list): Medicine[] {
  return list.filter((m) => {
    ...
    if (filters.purchaseStatus && getMedicinePurchaseStatus(m.id, purchases, medicines) !== filters.purchaseStatus) return false;
    if (filters.search) {
